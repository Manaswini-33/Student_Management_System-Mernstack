...
router.put("/faculty/:id", async (req, res) => {
    try {
        ...
        if (updates.email || updates.name || updates.department) {
            ...
            userUpdates.email = (updates.email || faculty.email).toLowerCase().trim();
        ...
        if (Object.keys(userUpdates).length > 0) {
            await User.findOneAndUpdate({ email: { $regex: new RegExp(`^${oldEmail}$`, "i") } }, userUpdates);
        }
...
router.delete("/faculty/:id", async (req, res) => {
    try {
        ...
        await User.findOneAndDelete({ email: { $regex: new RegExp(`^${faculty.email}$`, "i") } });
